...
class GUI:
    ...
    def __init__(self):
        ...
        self.__main_frame = Frame(self.__root, width=self.__width, height=self.__height)
        self.__main_frame.place(x=0, y=0)

        self.__canvas = Canvas(self.__main_frame, width=self.__width, height=self.__height, bd=10, bg="white")
        self.__canvas.place(x=0, y=0)

        self.__scrollbar = ttk.Scrollbar(self.__canvas, orient=VERTICAL, command=self.__canvas.yview)
        self.__scrollbar.place(x=10, y=200, anchor="w")

        self.__canvas.configure(yscrollcommand=self.__scrollbar.set)
        self.__canvas.bind('<Configure>', lambda e: self.__canvas.configure(scrollregion=self.__canvas.bbox("all")))

        self.__inner_frame = Frame(self.__canvas)#, width=self.__width, height=self.__height)
    ...
    def starting_buttons(self):
        ...
        # print(self.__width)
        search_button = Button(self.__main_frame, text="إبحث عن مريض", font=("Times", 20), command=self.__search_button_func)
    ...
    def create_therapy(self, i):
        ...
        therapy_label.place(x=LABELS_X, y=THERAPY_Y + 10 + i * 100, anchor="w")
        therapy_options = OptionMenu(self.__canvas, therapy_num, *THERAPYS_NUMS)
        therapy_options.place(x=WIDGETS_X, y=THERAPY_Y + 4 + i * 100, anchor="ne")
        ...
        therapy_date = Entry(self.__canvas, textvariable=date_value, width=20, font=("Times", 15),
                             highlightthickness=5, justify="right")
        therapy_date.place(x=WIDGETS_X - 90, y=THERAPY_Y + i * 100, anchor="ne")
        ...
        therapy_text.place(x=WIDGETS_X, y=THERAPY_Y + 40 + i * 100, anchor="ne")

        therapy_done_button = Button(self.__canvas, width=3, text="تم")
        therapy_done_button.place(x=WIDGETS_X - 350, y=THERAPY_Y + 5 + i * 100, anchor="ne")
    ...
    def create_all(self):
        ...
        for i in range(8):
            therapy_label, therapy_text, therapy_num, therapy_date_entry, date_value = self.create_therapy(i)
